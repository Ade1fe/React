...
const loadMessages = async (conversationId: string) => {
  ...
  return (
   ...
   <Box className=""  mb={4}>
   <Text fontSize="2xl" textAlign='center'>Chat with {selectedUser?.username ?? "Unknown"}</Text>

   <Text fontSize='xs' textAlign='center'> chat is probably available , you may need to click on the user and wait few seconds to load</Text>
   </Box>
        ...
        <Box display={['none', 'block']}   mr={4}>
          {users.map((user) => (
            <Box
              ...
              noOfLines={1}
            >
              <Text  textTransform='capitalize' fontWeight={selectedUser === user ? 'bold' : 'normal'}>{user.username}</Text>
            </Box>
          ))}
        </Box>
